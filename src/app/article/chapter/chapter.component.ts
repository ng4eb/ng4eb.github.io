import { Component, OnInit, ChangeDetectionStrategy, ViewChildren, ChangeDetectorRef, OnDestroy } from '@angular/core';
...
@Component({
    ...
    template: ` <markdown [src]="markdownPath" (ready)="onLoad($event)" #md> </markdown> `,
    styles: [],
...
export class ChapterComponent implements OnInit, OnDestroy {
    ...
    onLoad(_e: any) {
        setTimeout(() => {
            ...
            this._route.queryParamMap.pipe(takeUntil(this._destroy$)).subscribe((paramMap) => {
        ...
        if (this._isPlatformBrowserService.getIsPlatformBrowser()) {
            this._routingService
                ...
                .subscribe((url: string) => {
                    ...
                    const partNum = latterHalfInSegments[1].replace('p', '').split('?')[0].split('#')[0];
