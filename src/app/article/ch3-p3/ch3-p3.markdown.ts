export const ch3P3Markdown = `
## Overview of lifecycles
*The concept of a lifecycle applies to directives. In this presentation, we will focus on the lifecycles of components. However, the same concepts and techniques apply to all directives.*
...
The \`OnInit\` event will happen in all components. During this event, we will often put in logic such as making HTTP requests, performing side effects, and subscribing to observables. We will talk about all these in detail later.

The \`OnChanges\` event will only happen in components using the \`@Input()\` decorator. It will occur before the \`OnInit\` event.

\`OnChanges\` happens whenever Angular detects a change in the property bound by \`@Input()\`. As the parent component gets initialized, the data bound by \`@Input()\` will get initialized before the child component. Hence, \`OnChanges\` will occur first. Then, as the child component get initialized, the \`OnInit\` event will take place.

While \`OnInit\` will take place exactly once, \`OnChanges\` will happen whenever there is a change in the \`@Input()\` data from the parent.
...
Because \`DoCheck\` gets called frequently, it can be computationally expensive to use. Generally, this event should only be used (as the last resort) if there are changes that Angular cannot detect. You can read more about it in [the official documentation](https://angular.io/guide/lifecycle-hooks#docheck).
...
The \`AfterContentChecked\` event will take place immediately after \`AfterContentInit\` initially, and immediately after \`DoCheck\` subsequently.

#### AfterViewInit & AfterViewChecked

The \`AfterViewInit\` event will only occur once. It happens when a component's **view** is attached. In Angular, a **view** is like an organization of component(s). It helps to think of **view** in a nesting relationship.
...
In the end, a component may leave the DOM tree. This can be because we detach it using a structural directive like \`*ngIf\`, or we route to another page where the component does not exist. This is when the \`OnDestroy\` event will fire.

During the \`OnDestory\` event, we would commonly unsubscribe from observables to prevent memory leak.
