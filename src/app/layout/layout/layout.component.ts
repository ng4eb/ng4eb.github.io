...
import { filter, fromEvent, Observable, Subscription, switchMap, takeUntil } from 'rxjs';
...
export class LayoutComponent implements OnInit {
    ...
    constructor(private _layoutService: LayoutService, private _isPlatformBrowserService: IsPlatformBrowserService, private _el: ElementRef) {}
    ...
    register() {
        this.mousehold$ = this.touchstart$.pipe(
            switchMap(() => this.touchmove$),
            takeUntil(this.touchend$)
        );

        this._sub = this.mousehold$.pipe(filter((e) => e.touches[0].target.tagName !== 'PRE' && e.touches[0].target.tagName !== 'CODE')).subscribe((e) => {
            if (this.isFirstMove && window.getSelection()?.toString() === '' && Math.abs(this.y - e.touches[0].clientY) < 2.5) {
                this._layoutService.setSliding(true);
                this.detectXMovement(e.touches[0].clientX);
            } else if (!this.isFirstMove && this._layoutService.getIsSliding() && window.getSelection()?.toString() === '') {
