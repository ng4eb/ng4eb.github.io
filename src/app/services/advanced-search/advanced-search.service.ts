...
import {articleKey} from "../../article/seo";
import {HttpClient} from "@angular/common/http";
import {forkJoin, map, Observable} from "rxjs";
import {chapters} from "../../article/chapters";
...
export class AdvancedSearchService {
  ...
  loadMarkdownFiles(): Observable<Record<string, string>> {
    const observables = this._fileNames.map(fileName => this._http.get(`${this._markdownsFolder}${fileName}`, {responseType: 'text'})
      .pipe(map(content => ({
        [fileName.replace('.md', '')]: content
      }))));
    return forkJoin(observables).pipe(
      map(arrayOfObjects => arrayOfObjects.reduce((acc, obj) => ({...acc, ...obj}), {}))
    );
  }
