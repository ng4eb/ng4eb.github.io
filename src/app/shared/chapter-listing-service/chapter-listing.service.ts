...
export class ChapterListingService {
    ...
    getFilteredListing(filterQuery: string) {
        ...
        return this._chapterListing.filter(
            (chapter) =>
                chapter.title.toLowerCase().includes(filterQuery) ||
                chapter.parts.some((part) => part.title.toLowerCase().includes(filterQuery) || part.sections?.some((section) => section.toLowerCase().includes(filterQuery)))
        );
    }
